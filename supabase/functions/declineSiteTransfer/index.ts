...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!site_id) {
      return Response.json({ error: 'Site ID is verplicht' }, { status: 400, headers: corsHeaders })
    }
    // Get the site
    const { data: site, error: siteError } = await supabase.from('sites').select('*').eq('id', site_id).single()
    if (siteError || !site) {
      return Response.json({ error: 'Site niet gevonden' }, { status: 404, headers: corsHeaders })
    }
    // Verify user is the site owner
    if (site.owner_type !== 'user' || site.owner_id !== user.id) {
      return Response.json({ error: 'Je bent niet gemachtigd om dit verzoek af te handelen' }, { status: 403, headers: corsHeaders })
    }
    // Check if there's a pending transfer request
    if (!site.transfer_request || site.transfer_request.status !== 'pending') {
      return Response.json({ error: 'Geen openstaand overdrachtverzoek gevonden' }, { status: 400, headers: corsHeaders })
    }
    ...
    if (updateError) {
      return Response.json({ error: 'Kon overdrachtverzoek niet bijwerken' }, { status: 500, headers: corsHeaders })
    }
    ...
    return Response.json({ success: true, message: 'Overdrachtverzoek afgewezen' }, { headers: corsHeaders })
  } catch (error) {
    console.error('Decline site transfer error:', error)
    return Response.json({ error: error.message || 'Failed to decline site transfer' }, { status: 500, headers: corsHeaders })
  }
})